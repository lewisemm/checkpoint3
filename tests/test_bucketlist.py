...
class TestBucketList(TestBaseClass):
	...
	def test_successful_create_bucketlist(self):
		...
		self.client.post('/bucketlists/', data={'name': buck_name}, headers={'username': token})
	...
	def test_unauthenticated_create_bucketlist(self):
		"""
		Tests attempt to create a bucketlist without the authentication token.
		"""
		# create a user
		self.create_user()
		# log them in
	...
	def test_successful_authenticated_get_bucketlists(self):
		"""
		Tests successful attempt to get bucketlists in the system.
		"""
		# create a user
		self.create_user()
		# login user
		self.client.post('/auth/login', {'username': self.username, 'password': self.password}, headers={'username': token})
		# create a bucketlist
		buck_name = self.fake.name()
		self.client.post('/bucketlists/', data={'name': buck_name}, headers={'username': token})
		# create another bucketlist
		buck_name2 = self.fake.name()
		self.client.post('/bucketlists/', data={'name': buck_name2}, headers={'username': token})
	...
	def test_unauthenticated_get_bucketlists(self):
		"""
		Tests unauthenticated attempt to get bucketlists in the system.
		"""
		# create a user
		self.create_user()
		# login user
