...
class TestBucketListItemsId(TestBaseClass):
	...
	def test_successful_put_bucketlist_items(self):
		...
		self.client.post('/bucketlists/1', data={'name': item_name}, headers={'username': token})
		# update the item
		new_item_name = self.fake.name()
		self.client.post('/bucketlists/1/items/1', data={'name': new_item_name}, headers={'username': token})
	...
	def test_unauthenticated_put_bucketlist_items(self):
		...
		# update the item
		new_item_name = self.fake.name()
		self.client.post('/bucketlists/1/items/1', data={'name': new_item_name})
	...
	def test_successful_delete_bucketlist_items(self):
		...
		self.assertTrue('Item of id 1 in bucketlist of id 1 has been deleted' in response.content)
		self.assertEqual(response.status_code, 200)
